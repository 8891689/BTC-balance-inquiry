...
bool decompress_pubkey_bytes(const uint8_t compressed_key_bytes[33], ECPoint *point_out) {
    if (!(compressed_key_bytes[0] == 0x02 || compressed_key_bytes[0] == 0x03)) {
        return false; // 無效的前綴
    }

    bytes_be_to_bigint(compressed_key_bytes + 1, &point_out->x); // 獲取 X 坐標

    BigInt x3, x3_plus_7, y_candidate, seven_bi, p_plus_1_div_4_bi; // 需要初始化的 BigInt
    
    // 計算 THREE_BI (值為3) 和 p_plus_1_div_4_bi
    BigInt three_bi; init_bigint(&three_bi, 3);
    // (p+1)/4 的計算
    BigInt p_plus_1;
    BigInt one_bi; init_bigint(&one_bi, 1);
    ptx_u256Add(&p_plus_1, &secp256k1_p, &one_bi); // p+1
    // 簡單的右移2位實現除以4 (假設 BigInt 支持右移操作，或手動實現)
    // p_plus_1_div_4_bi = p_plus_1 >> 2; // 概念上的右移
    // 如果沒有直接右移，需要手動實現大數除以4
    // 簡化：假設我們有了 p_plus_1_div_4_bi
    // 例如，對於 secp256k1_p，(P+1)/4 = 0x3fffffffffffffffffffffffffffffffffffffffffffffffffffffffbfffff0c
    hex_to_bigint("3fffffffffffffffffffffffffffffffffffffffffffffffffffffffbfffff0c", &p_plus_1_div_4_bi);


    modexp(&x3, &point_out->x, &three_bi, &secp256k1_p); // x^3
    init_bigint(&seven_bi, 7);
    add_mod(&x3_plus_7, &x3, &seven_bi, &secp256k1_p);  // x^3 + 7

    // 計算 y_candidate = (x^3 + 7)^((p+1)/4) mod p
    modexp(&y_candidate, &x3_plus_7, &p_plus_1_div_4_bi, &secp256k1_p);

    // 驗證 y_candidate^2 == x^3 + 7 mod p
    BigInt y_candidate_squared;
    mul_mod_old(&y_candidate_squared, &y_candidate, &y_candidate, &secp256k1_p); // 或 mul_mod
    if (compare_bigint(&y_candidate_squared, &x3_plus_7) != 0) {
        point_out->infinity = true;
        return false; // 不是二次剩餘，或計算錯誤
    }

    // 根據前綴選擇正確的 Y
    bool y_is_odd = is_odd(&y_candidate);
    bool prefix_wants_odd = (compressed_key_bytes[0] == 0x03);

    if (y_is_odd != prefix_wants_odd) {
        ptx_u256Sub(&point_out->y, &secp256k1_p, &y_candidate); // y = p - y_candidate
    } else {
        copy_bigint(&point_out->y, &y_candidate);
    }
    point_out->infinity = false;
    return true;
}
